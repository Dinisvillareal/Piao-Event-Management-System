...
//   );
// }
//Last working code implementation ends here
import React, { useState, useEffect } from "react";
import { Camera, CameraOff, CheckCircle, XCircle, LogIn, LogOut } from "lucide-react";
import { Scanner } from '@yudiel/react-qr-scanner';
import { Card, CardHeader, CardTitle, CardDescription, CardContent, Button, Input, Label, Select, SelectItem, Badge } from "../../../Components/UI/Core";
...
export default function ScanView({ events, residents, memberships }: any) {
  ...
  // ✅ FIXED: Now uses ONLY user_id from QR, fetches everything else from DATABASE
// ✅ FIXED: Uses user_id from QR, fetches memberships from DATABASE
const handleQRCodeScan = (qrString: string) => {
  try {
    const data = JSON.parse(qrString);
    
    // Validate required fields from QR
    if (!data.user_id) {
      showModal('error', 'Invalid QR Code', 'Missing user ID in QR code.');
      return;
    }

    // Find resident in database using user_id from QR
    const matchedResident = residents?.find((r: any) => {
      if (r.id == data.user_id || r.user_id == data.user_id || r.real_id == data.user_id) return true;
      const checkString = r.user_code || r.id || "";
      if (typeof checkString === "string") {
        const stripped = checkString.replace("PR-", "").replace("RES-", "");
        if (parseInt(stripped, 10) == data.user_id) return true;
    ...
    // If resident not found in database, reject the scan
    if (!matchedResident) {
      showModal('error', 'Resident Not Found', `No resident found with ID: ${data.user_id}`);
      return;
    }

    // ✅ Get memberships from DATABASE (not from QR)
    ...
    // Get other data from database (with QR fallback only if needed)
    const residentName = matchedResident.first_name && matchedResident.last_name
      ...
      : data.name || `Resident #${data.user_id}`;
    
    const userCode = matchedResident.user_code || data.user_code || `ID: ${data.user_id}`;
    const photo = matchedResident.photo || matchedResident.validation_id_url;
    const role = matchedResident.role || "Resident";

    // ✅ Check event eligibility using DATABASE memberships
    let hasAccess = true;
    let reason = "Open event — all residents allowed";
    ...
    if (requiredMemberships && requiredMemberships.length > 0) {
      ...
      if (liveMemberships.length === 0) {
        hasAccess = false;
        reason = `❌ No memberships found. Requires: ${requiredNames.join(" or ")}`;
      } else {
        hasAccess = liveMemberships.some((userMem: string) => 
          requiredNames.includes(userMem)
        );
        reason = hasAccess 
          ? `✅ Eligible: ${liveMemberships.join(", ")}` 
          : `❌ Requires: ${requiredNames.join(" or ")} (Has: ${liveMemberships.join(", ") || "none"})`;
      }
    }
    ...
    setScan({
      ok: true,
      residentId: String(data.user_id),
